(function () {
  ...
  function isMojibake(text) {
    ...
    const globalArtifacts = /锘|谩|芒|茫|盲|猫|茅|锚|靓|貌|贸|么|玫|霉|煤|膼|膽|ďťż|Ã©|Ã³|Ã¡|Ã±|â€|Ðœ|Ð¾/i;
    if (globalArtifacts.test(text)) return true;

    if (text.includes('锘') || text.includes('谩')) return true;
    if (text.includes('ďťż')) return true;
    if (text.includes('???')) return true;
    if (text.includes('\uFFFD')) return true;

    if (/[a-zA-Z][\uFF61-\uFF9F]+[a-zA-Z]/.test(text)) return true;

    if (/[a-zA-Z][\u4E00-\u9FFF]+[a-zA-Z]/.test(text)) return true;

    if (/[\u4E00-\u9FA5]/.test(text)) {
      let hasHiddenSpace = false;
      let hasInvalidVietnameseByte = false;
      ...
      for (let i = 0; i < text.length; i += 1) {
        ...
        if (isInvalidByte1 || isInvalidByte2) {
          hasInvalidVietnameseByte = true;
        }
      }

      if (hasHiddenSpace && !hasInvalidVietnameseByte) {
        return true;
      }
    }
  ...
  function pickTitle(item) {
    ...
    if (!text || isMojibake(text)) {
      const pathObj =
        ...
        item.streamUrl ||
        '';
      const fallbackTitle = extractTitleFromPath(pathObj);
      if (fallbackTitle) {
  ...
  function pickArtist(item) {
    const raw =
      ...
      '';
    let text = String(raw).trim();
    if (!text || isUnknownArtist(text)) return '';

    text = decodeMojibakeString(text);
    text = text.replace(/(\w)Ž(?=\s|$)/g, '$1®');
    text = text.replace(/[\uFEFF\uFFFD]/g, '').trim();
    if (isUnknownArtist(text)) return '';
    return text;
  }
