...
#### Using HTTPS with Caddy (Recommended for local network)

If you encounter "HTTP block" or Mixed Content errors, you can use Caddy as a reverse proxy to provide local HTTPS:

1. Download Caddy (Windows amd64 version) from [caddyserver.com](https://caddyserver.com/).
2. Place the downloaded executable in this project folder and rename it to `caddy.exe`.
3. Start the Vite dev server (`npm run dev`) or Node server (`npm start`).
4. In a new terminal, run: `npm run proxy`
5. Access the app securely at `https://localhost` (Vite dev server) or `https://localhost:3443` (Node server).
...
- Access http://lsclient.qzz.io (HTTP, not HTTPS) to make it work, or use the provided Electron app
